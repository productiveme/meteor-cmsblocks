...
_CMSBlocks.prototype.configure = function({ onHasContent, onGetContent, onPermissionCheck, onUpdate, formClassNames, buttonClassNames }) {
   this.hasContentCallback.set(onHasContent);
   this.getContentCallback.set(onGetContent);
   this.permissionCheckCallback.set(onPermissionCheck);
   this.updateCallback.set(onUpdate);
   this.formClassNames.set(formClassNames);
   this.buttonClassNames.set(buttonClassNames);
};
...
if (Meteor.isClient) {
   ...
   Template.cmsblocks_container.helpers({
      ...
      showIcon() {
         return Session.get("cmsblocks.editmode") && !CMSBlocks.getContent(this.id || this.toString()).trim();
      },
      canEdit() {
