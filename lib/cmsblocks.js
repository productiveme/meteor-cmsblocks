...
if(Meteor.isClient) {
  ...
  Template.cmsblocks_container.helpers({
    ...
    showIcon: function() {
      return Session.get("cmsblocks.editmode") && !Template.instance().view.templateContentBlock.renderFunction()._render().value.trim();
    },
    canEdit: function() {
  ...
  Template.cmsblocks_container.events({
    ...
    "click .cmsblocks_hover": function(ev, tmpl) {
      ...
      editing.set(_.extend(this, { content: tmpl.view.templateContentBlock.renderFunction()._render().value }));
    }
  });
